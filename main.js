...
function drawHands(width) {
  ...
  if (mode & 4)
    drawLine(
      new Vector2(0, 0),
      Vector2.fromAngle(hours / 12, CLOCK_RADIUS * hourRatio)
    );
  if (mode & 2)
    drawLine(
      new Vector2(0, 0),
      Vector2.fromAngle(minutes / 60, CLOCK_RADIUS * minRatio)
    );
  if (mode & 1)
    drawLine(
      new Vector2(0, 0),
      Vector2.fromAngle(seconds / 60, CLOCK_RADIUS * secRatio)
    );
  ctx.stroke();
}
...
document.onkeydown = function (event) {
  switch (event.key) {
    ...
    case "u":
      ...
      break;
    case "h":
      hourRatio = hourRatio == 0.5 ? 0.707 : 0.5;
      break;
    case "s":
      shiftColours = !shiftColours;
      break;
    case "m":
      toggleMode();
      break;
